        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="<?php echo url('/admin/home');?>">Home</Mobiles></li>
              <li class="breadcrumb-item"><a href="<?php echo url('/admin/mobile');?>">Mobile</a></li>
              <li class="breadcrumb-item active">Edit Mobiles</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
          ...
          <div class="col-md-12">
            ...
            <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">Edit Mobile</h3>
              </div>
			 
              <!-- /.card-header -->
              <!-- form start -->
			   <form id="main_id" method="POST" action="<?php echo URL::to('/')?>/admin/mobile/{{App\Helpers\CryptHelper::encryptstring($data->id)}}" enctype="multipart/form-data">
				@method('PUT')
				@csrf
                <div class="card-body">
					<div class="row">
					     <div class="form-group col-md-6">
							 <label for="exampleInputEmail1">Category <span class="error">*</span></label>
							  ...
							  <span class="error" id='category_error'>{{$errors->Electric->first('category_fk')}}</span>
						</div>
						<div class="form-group col-md-6">
							 <label for="exampleInputEmail1">Brand <span class="error">*</span></label>
							  <select class="form-control" id="brand" name="brand_fk">
								<option value="">select brand</option>
								<?php foreach($brand as $retrieved_data) { ?>
								<option <?php if($data->brand_fk == $retrieved_data->id) { echo "selected";}?> value="{{$retrieved_data->id}}">{{$retrieved_data->name}}</option>
								<?php } ?>
							  </select>
							  <span class="error" id='brand_error'>{{$errors->Mobile->first('name')}}</span>
						</div>
						<div class="form-group col-md-6">
							 <label for="exampleInputEmail1">Name <span class="error">*</span></label>
							  ...
							  <span class="error" id='name_error'>{{$errors->Mobile->first('name')}}</span>
						</div>
					...
					<div class="form-group">
						...
						<span class="error" id='description_error'>{{$errors->Mobile->first('description')}}</span>
					</div>
					<div class="form-group mb-0">
						<div class="row">
							<div class="col-md-4">
								...
								<img class="mar-top-10 rp-img100" src="<?php echo url('/uploads/mobile/'.$data->image_1);?>" id="blah_1"/ ></br>
								<span class="error" id='image_1_error'>{{$errors->Mobile->first('image_1')}}</span>
							</div>
							<div class="col-md-4">
								<label for="exampleInputFile">Image 2</label>
								<input type="file" class="form-control" name="image_2" id="image_2">
								<img class="mar-top-10 rp-img100" <?php if($data->image_2) { ?> src="<?php echo url('/uploads/mobile/'.$data->image_2);?>" <?php } ?> id="blah_2"/ ></br>
								<span class="error" id='image_2_error'>{{$errors->Mobile->first('image_2')}}</span>
							</div>
							<div class="col-md-4">
								<label for="exampleInputFile">Image 3</label>
								<input type="file" class="form-control" name="image_3" id="image_3">
								<img class="mar-top-10 rp-img100" <?php if($data->image_3) { ?> src="<?php echo url('/uploads/mobile/'.$data->image_3);?>" <?php } ?> id="blah_3"/ ></br>
								<span class="error" id='image_3_error'>{{$errors->Mobile->first('image_3')}}</span>
							</div>
						</div>
					</div>
                </div>
...
/*Image 3 read url*/
  ...
  function ValidateSize(file) {
   ...
   $('#main_id').submit(function (e) {
	...
	$('#image_1_error').html("");
	$('#brand').removeClass("autofocous");
	$('.note-frame').removeClass("autofocous");
	$('#image_1').removeClass("autofocous");

   if (brand.trim() == '') {
		$('#brand_error').html("Please select Brand");
		cnt = 1;
		f++;
		if(f == 1)
		{
