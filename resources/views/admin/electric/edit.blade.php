        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item"><a href="<?php echo url('/admin/electric');?>">Products</a></li>
        ...
        <div class="row">
          ...
          <div class="col-md-12">
            ...
            <div class="card card-primary">
              ...
              <!-- /.card-header -->
              <!-- form start -->
			   <form id="main_id" method="POST" action="<?php echo URL::to('/')?>/admin/electric/{{App\Helpers\CryptHelper::encryptstring($data->id)}}" enctype="multipart/form-data">
				@method('PUT')
				@csrf
                <div class="card-body">
					<div class="row">
						<div class="form-group col-md-4">
							 <label for="exampleInputEmail1">Category <span class="error">*</span></label>
							  <select onchange="gettype(this.value);" class="form-control" id="category" name="category_fk">
							  ...
							  <span class="error" id='category_error'>{{$errors->Electric->first('name')}}</span>
						</div>
						<div class="form-group col-md-4">
							 <label for="exampleInputEmail1">Type <span class="error">*</span></label>
							  <select class="form-control" id="type" name="type_fk">
								<option value="">select type</option>
							  </select>
							  <span class="error" id='type_error'>{{$errors->Electric->first('type_fk')}}</span>
						</div>
						<div class="form-group col-md-4">
							 <label for="exampleInputEmail1">Name <span class="error">*</span></label>
							  ...
							  <span class="error" id='name_error'>{{$errors->Electric->first('name')}}</span>
						</div>
						<div class="form-group col-md-4">
					...
					<div class="form-group">
						...
						<span class="error" id='description_error'>{{$errors->Electric->first('description')}}</span>
					</div>
					<div class="form-group mb-0">
						<div class="row">
							<div class="col-md-4">
								...
								<img class="mar-top-10 rp-img100" src="<?php echo url('/uploads/electric/'.$data->image_1);?>" id="blah_1"/ ></br>
								<span class="error" id='image_1_error'>{{$errors->Electric->first('image_1')}}</span>
							</div>
							<div class="col-md-4">
								<label for="exampleInputFile">Image 2</label>
								<input type="file" class="form-control" name="image_2" id="image_2">
								<img class="mar-top-10 rp-img100" <?php if($data->image_2) { ?> src="<?php echo url('/uploads/electric/'.$data->image_2);?>" <?php } ?> id="blah_2"/ ></br>
								<span class="error" id='image_2_error'>{{$errors->Electric->first('image_2')}}</span>
							</div>
							<div class="col-md-4">
								<label for="exampleInputFile">Image 3</label>
								<input type="file" class="form-control" name="image_3" id="image_3">
								<img class="mar-top-10 rp-img100" <?php if($data->image_3) { ?> src="<?php echo url('/uploads/electric/'.$data->image_3);?>" <?php } ?> id="blah_3"/ ></br>
								<span class="error" id='image_3_error'>{{$errors->Electric->first('image_3')}}</span>
							</div>
						</div>
					</div>
                </div>
...
gettype('{{$data->category_fk}}');
function gettype(id)
{
	var type = '{{$data->type_fk}}';
	$.ajax({
		url: "<?php echo URL::to('/'); ?>/admin/get-type-by-category",
		type: "POST",
